...
const flattenSearchItems = (
  nodes: NavNode[],
  section = t("public.docs.documentation"),
): SearchItem[] =>
  nodes.flatMap((node) => [
...
const docsToc = inject<ComputedRef<Array<{ id: string; text: string }>>>(
  "docsToc",
  computed(() => []),
);
</script>
...
<template>
  <div
    class="docs-shell relative min-h-screen bg-[hsl(var(--site-bg))] font-docs text-[hsl(var(--site-text))]"
  >
    ...
    <header
      class="sticky top-0 z-50 w-full border-b border-[hsl(var(--site-border))] bg-[hsl(var(--site-bg))]/88 backdrop-blur-xl"
    >
      <div
        class="h-0.5 w-full bg-gradient-to-r from-[hsl(var(--site-accent))] via-cyan-400 to-transparent"
      />
      <div class="flex h-14 items-center justify-between px-4 md:px-6">
        <div class="flex items-center gap-6">
          ...
          <button
            ...
            :aria-label="t('public.docs.openNavigation')"
          ...
          <NuxtLink
            to="/"
            class="flex items-center gap-2.5 text-[hsl(var(--site-text))]"
          >
            <span
              class="flex h-7 w-7 items-center justify-center rounded-md bg-[hsl(var(--site-text))] font-docs-mono text-xs font-semibold text-[hsl(var(--site-bg))]"
              >L</span
            >
            <span class="text-lg font-semibold tracking-[-0.03em]"
              >launchctl</span
            >
            <span
              class="hidden rounded-full border border-[hsl(var(--site-border))] px-2 py-0.5 font-docs-mono text-[9px] uppercase tracking-[0.14em] text-[hsl(var(--site-text-muted))] sm:inline"
              >{{ t("public.navigation.docs") }}</span
            >
          </NuxtLink>
        ...
        <div class="flex items-center gap-3">
          <button
            class="hidden h-9 min-w-52 items-center gap-2 rounded-lg border border-[hsl(var(--site-border))] bg-[hsl(var(--site-surface))]/70 px-3 font-docs-mono text-xs text-[hsl(var(--site-text-muted))] shadow-sm transition-all hover:-translate-y-px hover:border-[hsl(var(--site-accent))]/40 hover:bg-[hsl(var(--site-surface-elevated))] hover:text-[hsl(var(--site-text))] md:flex"
            @click="openSearch"
          >
            <Icon name="lucide:search" class="h-3.5 w-3.5" />
            <span>{{ t("public.docs.search") }}</span>
            <kbd
              class="pointer-events-none ml-auto hidden h-5 select-none items-center rounded border border-[hsl(var(--site-border))] bg-[hsl(var(--site-bg))] px-1.5 text-[10px] sm:flex"
              >⌘K</kbd
            >
          </button>
          ...
          <a
            ...
            target="_blank"
            :aria-label="t('public.docs.openGithub')"
    ...
    <Teleport to="body">
      <div
        v-if="searchOpen"
        class="fixed inset-0 z-[100] flex items-start justify-center bg-slate-950/55 px-4 pt-[12vh] backdrop-blur-sm"
        role="dialog"
        aria-modal="true"
        :aria-label="t('public.docs.searchDocumentation')"
        @click.self="closeSearch"
      >
        <div
          class="w-full max-w-xl overflow-hidden rounded-2xl border border-[hsl(var(--site-border))] bg-[hsl(var(--site-bg))] font-docs text-[hsl(var(--site-text))] shadow-2xl shadow-black/25"
        >
          <div
            class="flex items-center gap-3 border-b border-[hsl(var(--site-border))] px-4"
          >
            <Icon
              name="lucide:search"
              class="h-5 w-5 shrink-0 text-[hsl(var(--site-accent))]"
            />
            <input
              ref="searchInput"
              v-model="searchQuery"
            ...
            >
            <kbd
              class="rounded border border-[hsl(var(--site-border))] px-1.5 py-1 font-docs-mono text-[10px] text-[hsl(var(--site-text-muted))]"
              >ESC</kbd
            >
          </div>
          <div class="max-h-[52vh] overflow-y-auto p-2">
            ...
            >
              <span>
                ...
                <span
                  class="mt-0.5 block font-docs-mono text-[10px] uppercase tracking-[0.12em] text-[hsl(var(--site-text-muted))]"
                  >{{ result.section }}</span
                >
              </span>
              <Icon
                name="lucide:arrow-up-right"
                class="h-4 w-4 text-[hsl(var(--site-text-muted))] opacity-0 transition-opacity group-hover:opacity-100"
              />
            </NuxtLink>
            <div
              v-if="searchResults.length === 0"
              class="px-4 py-12 text-center text-sm text-[hsl(var(--site-text-muted))]"
            >
              {{ t("public.docs.noResults", { query: searchQuery }) }}
            </div>
          </div>
          <div
            class="flex items-center justify-between border-t border-[hsl(var(--site-border))] bg-[hsl(var(--site-surface))]/50 px-4 py-2 font-docs-mono text-[10px] text-[hsl(var(--site-text-muted))]"
          >
            <span>{{
              t("public.docs.results", {
                count: formatNumber(searchResults.length),
              })
            }}</span>
            <span>{{ t("public.docs.searchHelp") }}</span>
          </div>
        </div>
      </div>
    </Teleport>
    ...
    >
      <div class="mb-6 flex items-center justify-between">
        <span
          class="font-docs-mono text-xs uppercase tracking-[0.2em] text-[hsl(var(--site-text-muted))]"
          >{{ t("public.docs.documentation") }}</span
        >
        <button
          class="text-[hsl(var(--site-text-muted))] hover:text-[hsl(var(--site-text))]"
          :aria-label="t('public.docs.closeNavigation')"
          @click="mobileSidebarOpen = false"
        >
      ...
      <nav>
        ...
        >
          ...
          <h4
            v-else
            class="mb-2 font-docs-mono text-xs uppercase tracking-[0.16em] text-[hsl(var(--site-text-muted))]"
          >
            {{ section.title }}
          </h4>
    ...
    <div class="mx-auto max-w-screen-2xl">
      <div class="flex">
        <!-- Desktop sidebar -->
        <aside
          class="hidden w-72 shrink-0 border-r border-[hsl(var(--site-border))] bg-[hsl(var(--site-surface))]/20 lg:block"
        >
          <div
            class="custom-logs-scrollbar sticky top-[3.625rem] h-[calc(100vh-3.625rem)] overflow-y-auto py-8 pr-5 pl-6"
          >
            <nav>
              <DocsNavItems
                v-if="topLevelItems.length"
              ...
              >
                ...
                <h4
                  v-else
                  class="mb-2 font-docs-mono text-[10px] font-medium uppercase tracking-[0.18em] text-[hsl(var(--site-text-muted))]"
                >
                  {{ section.title }}
                </h4>
                <DocsNavItems :items="section.items" />
        ...
        <aside class="hidden w-56 shrink-0 xl:block">
          <div class="sticky top-12 py-8 pl-4 pr-6">
            <div v-if="docsToc && docsToc.length > 0">
              <h4
                class="mb-4 flex items-center gap-2 font-docs-mono text-[10px] uppercase tracking-[0.16em] text-[hsl(var(--site-text-muted))]"
              >
                <span class="text-[hsl(var(--site-accent))]">#</span>
                {{ t("public.docs.onThisPage") }}
              </h4>
