...
const { connectExecution, disconnectAll } = useScriptExecution();
...
const initializeExecutions = async () => {
  try {
    const response = await $api<{ data: Execution[] }>(
      `/scripts/${props.scriptId}/executions`,
      {
    ...
    toast.error(t("operations.scripts.monitor.loadError"));
  } finally {
...
const getOutput = (serverId: string) => {
  return (
    outputRefs.value.get(serverId) ||
    executions.value.get(serverId)?.output ||
    ""
  );
};
...
const handleCopy = async (serverId: string) => {
  try {
    ...
    toast.success(t("operations.scripts.monitor.copied"));
  } catch {
    toast.error(t("operations.scripts.monitor.copyError"));
  }
};
      ...
      <SheetHeader class="flex-shrink-0 pb-4">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2">
            <Badge v-if="allCompleted" variant="success">
              {{ t("operations.scripts.common.status.complete") }}
            </Badge>
            <Badge v-else variant="secondary" class="animate-pulse">
              {{ t("operations.scripts.common.status.running") }}
            </Badge>
          </div>
        </div>
      </SheetHeader>
        ...
        >
          <TabsList class="w-full justify-start overflow-x-auto">
            ...
            >
              <span
                ...
                :class="
                  getStatusDot(getExecution(serverId)?.status || 'pending')
                "
              />
          ...
          >
            <div class="flex h-full flex-col gap-2">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                  <Badge
                    :variant="
                      getStatusBadge(
                        getExecution(serverId)?.status || 'pending',
                      ).variant
                    "
                  >
                    {{
                  ...
                  <span
                    v-if="
                      getExecution(serverId)?.exit_code !== null &&
                      getExecution(serverId)?.exit_code !== undefined
                    "
                    class="text-xs text-muted-foreground"
                  >
                    {{
                      t("operations.scripts.monitor.exitCode", {
                        code: getExecution(serverId)?.exit_code,
                      })
                    }}
                  </span>
                </div>
              ...
              <div
                class="min-h-0 flex-1 overflow-hidden rounded-lg bg-zinc-950"
              >
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              <Badge
                :variant="
                  getStatusBadge(
                    getExecution(activeServerId)?.status || 'pending',
                  ).variant
                "
              >
                {{
              ...
              <span
                v-if="
                  getExecution(activeServerId)?.exit_code !== null &&
                  getExecution(activeServerId)?.exit_code !== undefined
                "
                class="text-xs text-muted-foreground"
              >
                {{
                  t("operations.scripts.monitor.exitCode", {
                    code: getExecution(activeServerId)?.exit_code,
                  })
                }}
              </span>
            </div>
            <Button
              variant="ghost"
              size="sm"
              @click="handleCopy(activeServerId)"
            >
              <Icon name="lucide:copy" class="mr-2 h-4 w-4" />
              {{ t("operations.scripts.common.copy") }}
            </Button>
