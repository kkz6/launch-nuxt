...
const variants: Record<string, { box: string; icon: string; ic: string }> = {
  info: {
    box: "border-sky-500/30 bg-sky-500/[0.06]",
    icon: "lucide:info",
    ic: "text-sky-500",
  },
  warning: {
  ...
  note: {
    box: "border-[hsl(var(--site-border))] bg-[hsl(var(--site-surface))]",
    icon: "lucide:pencil-line",
    ic: "text-[hsl(var(--site-text-muted))]",
  },
};
...
<template>
  <div
    class="my-5 flex gap-3 rounded-lg border p-4 text-[hsl(var(--site-text-muted))]"
    :class="v.box"
  >
    <Icon :name="v.icon" class="mt-0.5 h-4 w-4 shrink-0" :class="v.ic" />
    <div
      class="min-w-0 text-sm leading-relaxed [&_p]:my-0 [&_p+p]:mt-2 [&_code]:text-[0.85em]"
    >
      <p v-if="title" class="mb-1 font-semibold text-[hsl(var(--site-text))]">
        {{ title }}
      </p>
      <slot />
    </div>
