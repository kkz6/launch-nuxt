...
const variants: Record<string, { box: string; icon: string; ic: string }> = {
  ...
  note: { box: 'border-[hsl(var(--site-border))] bg-[hsl(var(--site-surface))]', icon: 'lucide:pencil-line', ic: 'text-[hsl(var(--site-text-muted))]' },
}
const v = computed(() => variants[props.type] ?? variants.info)
</script>

<template>
  <div class="my-5 flex gap-3 rounded-lg border p-4 text-[hsl(var(--site-text-muted))]" :class="v.box">
    ...
    <div class="min-w-0 text-sm leading-relaxed [&_p]:my-0 [&_p+p]:mt-2 [&_code]:text-[0.85em]">
      <p v-if="title" class="mb-1 font-semibold text-[hsl(var(--site-text))]">{{ title }}</p>
      <slot />
    </div>
  </div>
</template>
