...
const cronPresets = computed<Record<string, { label: string; value: string }>>(
  () => ({
    hourly: { label: t("server.backupForm.hourly"), value: "0 * * * *" },
    daily: { label: t("server.backupForm.daily"), value: "0 0 * * *" },
    weekly: { label: t("server.backupForm.weekly"), value: "0 0 * * 0" },
    monthly: { label: t("server.backupForm.monthly"), value: "0 0 1 * *" },
  }),
);

const getSchema = () =>
  z.object({
    storage_provider_id: z
      .string()
      .min(1, t("server.backupForm.storageRequired")),
    cron_expression: z.string().min(1, t("server.backupForm.scheduleRequired")),
    path: z.string().min(1, t("server.backupForm.pathRequired")),
    retention: z.number().min(1, t("server.backupForm.retentionRequired")),
...
const onSubmit = async () => {
  ...
  const actionText = isEditing.value
    ? t("server.common.update")
    : t("server.common.create");
  const result = await confirmationDialog.value.show({
    title: t("server.backupForm.actionTitle", { action: actionText }),
    description: t("server.backupForm.actionDescription", {
      action: actionText,
    }),
  ...
  try {
    if (isEditing.value && props.backup) {
      ...
      toast.success(t("server.backupForm.updated"));
    } else {
      await $api(`/servers/${props.serverId}/backups`, {
        method: "POST",
        body: data,
      });
      toast.success(t("server.backupForm.created"));
    }
    emit("created");
    open.value = false;
    resetForm();
    ...
    toast.error(
      err.data?.message ||
        t("server.backupForm.actionFailed", { action: actionText }),
    );
  } finally {
  ...
  <Dialog v-model:open="open">
    <DialogTrigger as-child>
      <Button>
        ...
        {{ t("server.backupForm.createBackup") }}
      </Button>
    </DialogTrigger>
    <DialogContent class="sm:max-w-2xl">
      ...
      <DialogHeader>
        <DialogTitle>{{
          t("server.backupForm.dialogTitle", {
            action: isEditing
      ...
      <form class="grid w-full gap-4" @submit.prevent="onSubmit">
        ...
        <div class="space-y-2">
          <Label for="storage_provider_id">{{
            t("server.backupForm.storageProvider")
          }}</Label>
          <Select v-model="storageProviderId">
            <SelectTrigger>
              <SelectValue
                :placeholder="t('server.backupForm.selectStorage')"
              />
            </SelectTrigger>
            <SelectContent>
              <SelectItem
                v-for="provider in props.storageProviders"
                :key="provider.id"
          ...
          <p v-if="errors.storage_provider_id" class="text-sm text-destructive">
            {{ errors.storage_provider_id }}
          </p>
        </div>

        <!-- Path -->
        <div class="space-y-2">
          <Label for="path">{{ t("server.backupForm.path") }}</Label>
          <Input id="path" v-model="path" placeholder="/" />
          <p class="text-xs text-muted-foreground">
            {{ t("server.backupForm.pathHelp") }}
          </p>
        </div>

        <!-- Databases -->
        <div v-if="props.databases.length > 0" class="space-y-2">
          <Label>{{ t("server.backupForm.databases") }}</Label>
          <div class="grid grid-cols-2 gap-2 rounded-lg border p-3">
            ...
            >
              ...
              />
              <Label :for="`db-${db.id}`" class="font-normal">{{
                db.name
              }}</Label>
            </div>
          </div>
          <p
            v-if="props.databases.length === 0"
            class="text-xs text-muted-foreground"
          >
            {{ t("server.backupForm.noDatabases") }}
          </p>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="space-y-2">
            <Label>{{ t("server.backupForm.schedulePreset") }}</Label>
            <Select @update:model-value="cronExpression = String($event)">
              <SelectTrigger>
                <SelectValue
                  :placeholder="t('server.backupForm.selectPreset')"
                />
              </SelectTrigger>
              <SelectContent>
                <SelectItem
                  v-for="(preset, key) in cronPresets"
                  :key="key"
                  :value="preset.value"
                >
                  {{ preset.label }}
          ...
          <div class="space-y-2">
            <Label for="cron_expression">{{
              t("server.backupForm.cronExpression")
            }}</Label>
            <Input
              id="cron_expression"
        ...
        <!-- Retention -->
        <div class="space-y-2">
          <Label for="retention">{{ t("server.backupForm.retention") }}</Label>
          <Input
            id="retention"
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="space-y-3">
            <Label>{{ t("server.backupForm.notifications") }}</Label>
            <div class="space-y-2">
              <div class="flex items-center space-x-2">
                ...
                />
                <Label for="notification_on_failure" class="font-normal">{{
                  t("server.backupForm.notifyFailure")
                }}</Label>
              </div>
              <div class="flex items-center space-x-2">
                <Checkbox
                ...
                />
                <Label for="notification_on_success" class="font-normal">{{
                  t("server.backupForm.notifySuccess")
                }}</Label>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-3">
            <Label>{{ t("server.common.status") }}</Label>
            <div class="flex items-center space-x-2">
              <Checkbox id="enabled" v-model="enabled" />
              <Label for="enabled" class="font-normal">{{
                t("server.backupForm.enable")
              }}</Label>
            </div>
          </div>
        </div>
        ...
        <DialogFooter>
          ...
          <Button type="submit" :disabled="!canSubmit">
            ...
            {{
              isEditing
                ? t("server.backupForm.updateBackup")
                : t("server.backupForm.createBackup")
            }}
          </Button>
        </DialogFooter>
