...
import type {
  MetricsNetwork,
  MetricsData,
} from "~/composables/useMetricsStream";
...
<template>
  <div class="rounded-lg border border-border bg-card p-4">
    ...
    <div class="grid grid-cols-2 gap-4">
      <div>
        ...
        <p class="text-[10px] text-muted-foreground">
          {{
            t("server.metrics.total", {
              value: formatBytes(network?.rx_bytes || 0),
            })
          }}
        </p>
      </div>
      <div>
        ...
        <p class="text-[10px] text-muted-foreground">
          {{
            t("server.metrics.total", {
              value: formatBytes(network?.tx_bytes || 0),
            })
          }}
        </p>
      </div>
    ...
    <div
      v-if="rxHistory.length > 0 || txHistory.length > 0"
