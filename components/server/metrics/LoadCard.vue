...
<template>
  <div class="rounded-lg border border-border bg-card p-4">
    ...
    <div class="mt-1 flex gap-2 text-xs text-muted-foreground">
      <span
        >{{ t("server.metrics.oneMinute") }}:
        <span class="font-medium text-foreground">{{
          metrics?.load[0]?.toFixed(2) || "0.00"
        }}</span></span
      >
      <span
        >{{ t("server.metrics.fiveMinutes") }}:
        <span class="font-medium text-foreground">{{
          metrics?.load[1]?.toFixed(2) || "0.00"
        }}</span></span
      >
      <span
        >{{ t("server.metrics.fifteenMinutes") }}:
        <span class="font-medium text-foreground">{{
          metrics?.load[2]?.toFixed(2) || "0.00"
        }}</span></span
      >
    </div>
