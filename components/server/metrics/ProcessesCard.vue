  ...
  <div class="rounded-lg border border-border bg-card p-4">
    ...
    <div v-if="processes.length > 0" class="space-y-2">
      ...
      >
        ...
        <span class="flex-1 truncate font-medium" :title="process.command">{{
          process.command
        }}</span>
    ...
    <div v-else class="text-xs text-muted-foreground text-center py-4">
      {{ t("server.metrics.noProcessData") }}
    </div>
    <div
      class="mt-2 flex items-center text-[10px] text-muted-foreground border-t border-border pt-2"
    >
      <span class="w-16 shrink-0">PID</span>
      <span class="flex-1">{{ t("server.metrics.command") }}</span>
      <span class="w-16 shrink-0 text-right">{{
        t("server.metrics.user")
      }}</span>
      <span class="w-14 shrink-0 text-right">{{
        t("server.metrics.cpu")
      }}</span>
      <span class="w-14 shrink-0 text-right">{{
        t("server.metrics.memoryShort")
      }}</span>
    </div>
  </div>
</template>
