...
const applyRunEventToConsole = (data: BackupRunEventPayload, event: string) => {
  ...
  if (event === "backup.run.failed") {
    logSheetError.value =
      ...
      (logSheetTaskId.value
        ? ""
        : t("server.settings.backups.failedBeforeLogs"));
  } else {
    logSheetError.value = "";
  }
...
const scheduleRunReconciliation = (
  ...
  if (attempt >= RUN_RECONCILE_MAX_ATTEMPTS) {
    awaitingRunLogs.value = false;
    logSheetError.value = t("server.settings.backups.liveUnavailable");
    return;
  }
  runReconcileTimer = setTimeout(() => {
    runReconcileTimer = undefined;
    void reconcileRunConsole(
      backupId,
      jobId,
      serverId,
      requestToken,
...
async function reconcileRunConsole(
  ...
  if (!ownsRunReconciliation(jobId, requestToken)) return;
  try {
    const response = await $api<{ data: Backup }>(
...
const toastRunTerminal = (
  ...
  if (event === "backup.run.succeeded") {
    toast.success(t("server.settings.backups.completed"));
    return;
  }
  ...
  toast.error(
    hasTaskLogs
      ? t("server.settings.backups.failedCheckLogs")
      : failureMessage || t("server.settings.backups.failed"),
  );
};
...
const fetchBackups = async () => {
  ...
  try {
    ...
    if (fetchToken === activeBackupsFetchToken && serverId === props.serverId) {
      toast.error(t("server.settings.backups.loadFailed"));
    }
  } finally {
    if (fetchToken === activeBackupsFetchToken && serverId === props.serverId) {
...
const runBackup = async (backup: Backup) => {
  ...
  const result = await confirmationDialog.value.show({
    title: t("server.settings.backups.run"),
    description: t("server.settings.backups.runDescription"),
    confirmText: t("server.settings.backups.run"),
  ...
  try {
    const response = await $api<{ data: BackupJob }>(
      `/servers/${requestServerId}/backups/${backup.id}/run`,
      {
        method: "POST",
      },
    );
    ...
    if (!job?.id || job.backup_id !== backup.id) {
      throw new Error(t("server.settings.backups.invalidRunId"));
    }
    ...
    if (authoritativeTerminalEvent) {
      ...
      toast.success(t("server.settings.backups.started"));
    ...
    const message =
      e.data?.message || e.message || t("server.settings.backups.startFailed");
    if (
      serverContextToken !== activeServerContextToken ||
      requestServerId !== props.serverId
    )
...
const deleteBackup = async (backup: Backup) => {
  ...
  const result = await confirmationDialog.value.show({
    title: t("server.settings.backups.deleteTitle"),
    description: t("server.settings.backups.deleteDescription"),
  ...
  try {
    ...
    toast.success(t("server.settings.backups.deleted"));
  } catch {
    toast.error(t("server.settings.backups.deleteFailed"));
  } finally {
    loadingActions.value = { ...loadingActions.value, [backup.id]: "" };
  }
};
...
const getProviderName = (providerId: string) => {
  return storageProvidersMap.value[providerId] || t("server.common.unknown");
};

const getBackupSource = (backup: Backup) => {
  if (backup.databases && backup.databases.length > 0) {
    return t("server.settings.backups.databaseCount", {
      count: backup.databases.length,
    });
  }
  return backup.path || "/";
};
...
const columns = computed(() => [
  {
    key: "provider",
    label: t("server.settings.backups.provider"),
    width: "25%",
  },
  { key: "source", label: t("server.settings.backups.source"), width: "20%" },
  {
    key: "schedule",
    label: t("server.settings.backups.schedule"),
    width: "20%",
  },
  {
    key: "status",
    label: t("server.settings.backups.lastBackup"),
    width: "15%",
  },
]);
...
const actions = computed(() => [
  {
    label: t("server.settings.backups.run"),
    icon: "lucide:play",
    onClick: runBackup,
  },
  {
    label: t("server.settings.backups.viewHistory"),
    icon: "lucide:history",
    onClick: openHistory,
  },
  {
...
<template>
  <div class="space-y-6">
      ...
      >
        ...
        <div class="mt-4 flex flex-1 flex-col min-h-0">
          ...
          >
            <div class="max-w-lg space-y-2 text-destructive">
              ...
              <p class="text-sm font-medium">
                {{ t("server.settings.backups.outputUnavailable") }}
              </p>
          ...
          >
            ...
            {{ t("server.settings.backups.starting") }}
          </div>
        </div>
      </SheetContent>
    </Sheet>
    ...
    <ServerCreateBackup
      ...
      @update:open="
        (val: boolean) => {
          if (!val) selectedBackupForEdit = null;
        }
      "
    ...
    <div>
      <h3 class="text-lg font-medium">
        {{ t("server.settings.backups.title") }}
      </h3>
      <p class="text-sm text-muted-foreground">
        {{ t("server.backupForm.description") }}
      </p>
    </div>
    ...
    <template v-else>
      ...
      <div v-if="!hasProviders" class="flex flex-col items-center gap-3 py-8">
        ...
        <p class="text-center text-muted-foreground">
          {{ t("server.settings.backups.noProviders") }}
          <button
            type="button"
            class="text-foreground underline"
            @click="openSettings('connections')"
          >
            {{ t("server.settings.backups.addProvider") }}
          </button>
          {{ t("server.settings.backups.addProviderSuffix") }}
        </p>
      </div>
      ...
      <template v-else>
        <SharedDataTable
          ...
          :empty-title="t('server.settings.backups.empty')"
          :empty-description="t('server.settings.backups.emptyDescription')"
        >
          <template #empty>
            <ServerCreateBackup
              :server-id="serverId"
          ...
          <template #cell-provider="{ row }">
            <div class="flex items-center gap-2">
              <span class="font-medium">{{
                getProviderName(row.storage_provider_id)
              }}</span>
              <Badge v-if="!row.enabled" variant="secondary" class="text-xs">{{
              ...
              <Tooltip>
                <TooltipTrigger as-child>
                  <Badge
                    variant="outline"
                    :class="[
                      getStatusConfig(row.latest_job.status).class,
                      'cursor-help',
                    ]"
                  >
                    <Icon
                      ...
                      :class="[
                        'mr-1 h-3 w-3',
                        row.latest_job.status === 'running' && 'animate-spin',
                      ]"
                    />
                ...
                <TooltipContent class="max-w-xs">
                  <div class="space-y-1.5 text-sm">
                    <div class="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                      ...
                      <span
                        :class="
                          row.latest_job.status === 'finished'
                            ? 'text-green-500'
                            : row.latest_job.status === 'failed'
                              ? 'text-red-500'
                              : ''
                        "
                      >
                      ...
                      <span class="text-muted-foreground"
                        >{{ t("server.settings.backups.retention") }}:</span
                      >
                      <span>{{
                        t("server.settings.backups.backupCount", {
                          count: row.retention,
                        })
                      }}</span>
                      <span class="text-muted-foreground"
                        >{{ t("server.settings.backups.provider") }}:</span
                      >
                      <span>{{
                        getProviderName(row.storage_provider_id)
                      }}</span>
                    ...
                    <p
                      v-if="row.latest_job.error"
                      class="border-t pt-1.5 text-xs text-red-500"
                    >
                      {{ row.latest_job.error }}
                    </p>
                    <p class="border-t pt-1.5 text-xs text-muted-foreground">
                      {{ t("server.settings.backups.lastRun") }}:
                      <SharedDateTooltip :date="row.latest_job.created_at" />
                    </p>
                  </div>
                </TooltipContent>
              </Tooltip>
