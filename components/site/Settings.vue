...
const deletionResources = ref<{ queues: number; crons: number }>({
  queues: 0,
  crons: 0,
});
...
const fetchSettings = async (syncMode: FormSyncMode = "all") => {
  ...
  try {
    ...
    tlsOptions.value = (response.data.tls_options || []).reduce(
      (acc, opt) => {
...
const formatPhpVersionLabel = (label: string) =>
  label.replace(" (Default)", ` (${t("site.settings.defaultVersion")})`);
...
const notifyPhpUpdateResolved = (requestedVersion: string, error?: string) => {
  ...
  if (persistedPhpVersion.value === requestedVersion) {
    toast.success(t("site.settings.phpUpdated"));
  } else {
    toast.error(t("site.settings.phpUpdateFailed"));
  }
};
...
const onSubmit = handleSubmit(async (values) => {
  ...
  const result = await confirmationDialog.value.show({
    title: t("site.settings.confirmTitle"),
    description: t("site.settings.confirmDescription"),
  ...
  try {
    ...
    if (phpVersionChanged) {
      ...
      if (isPhpUpdatePending.value) {
        toast.success(
          t("site.settings.phpUpdateQueued", {
            version:
              pendingPhpVersionLabel.value || t("site.settings.phpVersion"),
          }),
        );
      } else {
      ...
      toast.success(t("site.settings.updated"));
    }
    emit("updated");
  } catch (error: unknown) {
    const err = error as {
    ...
    if (err.data?.errors) {
      ...
      toast.error(err.data?.message || t("site.settings.updateFailed"));
    }
  } finally {
...
useSiteEvents(teamId, async (data, eventName) => {
  ...
  const eventVersion = data.php_version
    ? phpVersionKey(data.php_version)
    : null;
  if (
    pendingPhpVersion.value &&
    eventVersion &&
  ...
  if (requestedVersion) {
    notifyPhpUpdateResolved(
      requestedVersion,
      eventName === "site.php_version_update_failed"
        ? data.error || t("site.settings.phpUpdateFailed")
        : undefined,
    );
  }
});
...
const deleteSite = async () => {
  ...
  const result = await confirmationDialog.value.show({
    title: t("site.settings.deleteTitle"),
    description: t("site.settings.deleteDescription", {
      site: props.site.address,
      queues: formatNumber(deletionResources.value.queues),
      crons: formatNumber(deletionResources.value.crons),
    }),
    confirmText: t("site.settings.delete"),
    cancelText: t("site.common.cancel"),
    destructive: true,
    helpText: t("site.settings.deleteHelp"),
  ...
  if (result.ok) {
    try {
      ...
      toast.success(t("site.settings.deleted"));
      emit("deleted");
      navigateTo(`/servers/${props.serverId}`);
    } catch (error: unknown) {
      const err = error as { data?: { message?: string } };
      toast.error(err.data?.message || t("site.settings.deleteFailed"));
    }
  }
};
  ...
  <div>
    ...
    <div class="space-y-6">
      ...
      <template v-if="site.type !== 'phpmyadmin'">
        <div>
          <h3 class="text-lg font-semibold">{{ t("site.settings.title") }}</h3>
          <p class="text-sm text-muted-foreground">
            {{ t("site.settings.description") }}
          </p>
        </div>
        ...
        <form class="space-y-6" @submit.prevent="onSubmit">
          <div class="grid grid-cols-2 gap-6">
            <FormField v-slot="{ componentField }" name="php_version">
              <FormItem>
                <FormLabel>{{ t("site.settings.phpVersion") }}</FormLabel>
                <Select v-bind="componentField" :disabled="isPhpUpdatePending">
                  <FormControl>
                    <SelectTrigger>
                      <SelectValue
                        :placeholder="t('site.settings.selectPhpVersion')"
                      />
                    </SelectTrigger>
                  </FormControl>
                  <SelectContent>
                  ...
                  <span class="inline-flex items-center gap-1.5">
                    ...
                    {{
                      pendingPhpVersionLabel
                        ? t("site.settings.switchingPhp", {
                            version: pendingPhpVersionLabel,
                          })
                        : t("site.settings.phpUpdateInProgress")
                    }}
                  </span>
                </FormDescription>
                <FormMessage />
            ...
            <FormField
              v-if="!['wordpress'].includes(site.type)"
              v-slot="{ componentField }"
              name="web_folder"
            >
              <FormItem>
                <FormLabel>{{ t("site.settings.webFolder") }}</FormLabel>
                <FormControl>
            ...
            <template v-if="repositoryData">
              <div class="space-y-2">
                <Label class="text-sm font-medium">{{
                  t("site.settings.repository")
                }}</Label>
                <div
                  class="flex items-center gap-2 rounded-sm border border-border bg-muted/40 p-3"
                >
                  <Icon
                  ...
                  <div class="min-w-0 flex-1">
                    ...
                    <p class="truncate text-xs text-muted-foreground">
                      {{ currentSourceControl?.displayName }} •
                      {{ currentSourceControl?.provider }}
                    </p>
                  </div>
                  <Button
              ...
              <FormField v-slot="{ componentField }" name="repository_branch">
                <FormItem>
                  <FormLabel>{{ t("site.settings.branch") }}</FormLabel>
                  <FormDescription class="mb-2 text-xs">
                    {{ t("site.settings.currentBranch") }}
                  ...
                  <FormControl>
                    <Input
                      :placeholder="repositoryData.default_branch || 'main'"
          ...
          <Button type="submit" :disabled="isLoading || isPhpUpdatePending">
            ...
            {{
              isPhpUpdatePending
                ? t("site.settings.updateInProgress")
                : t("site.settings.update")
            }}
          </Button>
        </form>
        <Separator />
      ...
      <div class="space-y-4">
        <div>
          <h3 class="text-lg font-medium">{{ t("site.settings.sslTitle") }}</h3>
          <p class="text-sm text-muted-foreground">
            {{ t("site.settings.sslDescription") }}
          </p>
        </div>
        ...
        <div class="space-y-4">
          <div>
            <h3 class="text-lg font-medium">
              {{ t("site.settings.deploymentTitle") }}
            </h3>
            <p class="text-sm text-muted-foreground">
              {{ t("site.settings.deploymentDescription") }}
            </p>
          </div>
        ...
        <div class="space-y-4 pt-2">
          <div>
            <h3 class="text-lg font-medium text-destructive">
              {{ t("site.settings.dangerTitle") }}
            </h3>
            <p class="text-sm text-muted-foreground">
              {{ t("site.settings.dangerDescription") }}
            </p>
          </div>
          <Button variant="destructive" @click="deleteSite">
            <Icon name="lucide:trash-2" class="mr-2 h-4 w-4" />
            {{ t("site.settings.delete") }}
          </Button>
        </div>
      </template>
    </div>
