...
>(() => ({
  ...
  running: {
    variant: "warning",
    label: t("site.status.running"),
    icon: "lucide:loader-2",
  },
  finished: {
    variant: "success",
    label: t("site.status.success"),
    icon: "lucide:check",
  },
  completed: {
    variant: "success",
  ...
  <div>
    <div class="mb-4 flex items-center justify-between">
      <div class="flex items-center gap-2">
        <h3 class="text-lg font-semibold">{{ t("site.deployBlock.title") }}</h3>
        <Badge
          v-if="site.zero_downtime_deployment"
          variant="outline"
          class="font-normal"
        >
          {{ t("site.deployBlock.zeroDowntime") }}
        </Badge>
      </div>
      <SiteAutodeploy
        :server-id="server.id"
        :site-id="site.id"
        :auto-deployment="site.auto_deployment"
      />
    </div>
    ...
    <div v-if="site.latest_deployment" class="rounded-lg border bg-card p-4">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          <Badge
            :variant="
              statusConfig[site.latest_deployment.status]?.variant ||
              'secondary'
            "
            class="gap-1"
          >
            <Icon
              ...
              :class="[
                'size-3',
                ['pending', 'installing', 'running'].includes(
                  site.latest_deployment.status,
                ) && 'animate-spin',
              ]"
            />
            {{
              statusConfig[site.latest_deployment.status]?.label ||
              site.latest_deployment.status
            }}
          </Badge>
        </div>
      ...
      <div v-if="site.latest_deployment.commit_data" class="mt-3 space-y-2">
        <!-- Commit message -->
        <p
          v-if="site.latest_deployment.commit_data.message"
        ...
        >
          <!-- Author -->
          <div
            v-if="site.latest_deployment.commit_data.name"
          ...
          <!-- Commit SHA -->
          <div
            v-if="site.latest_deployment.commit_data.sha"
            class="flex items-center gap-1.5"
          >
      ...
      <div
        v-if="
          site.latest_deployment?.task_id &&
          site.latest_deployment.status === 'pending'
        "
        class="mt-4 border-t pt-4"
      >
        <div class="mb-2 flex items-center justify-between">
          <h4 class="text-sm font-medium">{{ t("site.deployBlock.logs") }}</h4>
          <Button variant="ghost" size="sm" @click="showLogs = !showLogs">
            <Icon
              :name="showLogs ? 'lucide:chevron-up' : 'lucide:chevron-down'"
              class="mr-1.5 size-4"
            />
            {{ showLogs ? t("site.common.hide") : t("site.common.view") }}
          </Button>
        </div>
    ...
    <div
      v-else
      class="flex flex-col items-center justify-center gap-3 rounded-lg border border-dashed bg-card py-8 text-muted-foreground"
    >
      <Icon name="lucide:rocket" class="size-8" />
      <span class="text-sm">{{ t("site.deployBlock.empty") }}</span>
    </div>
  </div>
</template>
