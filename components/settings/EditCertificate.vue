...
const certTouched = computed(
  () =>
    certificatePem.value.trim().length > 0 ||
...
const validate = () => {
  ...
  if (certificatePem.value.trim() && !privateKey.value.trim()) {
    errors.value.private_key = t(
      "settings.certificates.rotationPrivateKeyRequired",
    );
  }
  if (privateKey.value.trim() && !certificatePem.value.trim()) {
    errors.value.certificate = t(
      "settings.certificates.rotationCertificateRequired",
    );
  }
...
const onSubmit = async () => {
  ...
  if (Object.keys(body).length === 0) {
    toast.info(t("settings.certificates.noChanges"));
    open.value = false;
  ...
  try {
    ...
    if (body.certificate) {
      ...
      if (pendingRedeploys > 0) {
        toast.success(
          t(
            pendingRedeploys === 1
              ? "settings.certificates.updatedRedeploySingle"
              : "settings.certificates.updatedRedeployMultiple",
            { count: pendingRedeploys },
          ),
        );
      } else {
        toast.success(t("settings.certificates.updated"));
      }
    ...
    if (e.response?.status === 422 && e.data?.errors) {
      ...
      toast.error(e.data?.message || t("settings.certificates.updateFailed"));
    }
  } finally {
    isLoading.value = false;
  }
...
<template>
  <Sheet v-model:open="open">
    <SheetContent class="w-full overflow-y-auto sm:max-w-xl">
      <SheetHeader>
        <SheetTitle>{{ t("settings.certificates.editTitle") }}</SheetTitle>
        <SheetDescription>
          {{ t("settings.certificates.editDescription") }}
        </SheetDescription>
      </SheetHeader>

      <form class="mt-6 space-y-4" @submit.prevent="onSubmit">
        <div class="space-y-1.5">
          <Label for="cert-edit-name">
            {{ t("settings.certificates.name") }}
          </Label>
          <Input
            id="cert-edit-name"
            v-model="name"
        ...
        <div class="space-y-3 rounded-md border border-dashed p-4">
          <div class="space-y-1">
            <p class="text-sm font-medium">
              {{ t("settings.certificates.rotateTitle") }}
            </p>
            <p class="text-[11px] text-muted-foreground">
              {{ t("settings.certificates.rotateDescription") }}
            </p>
          </div>

          <div class="space-y-1.5">
            <Label for="cert-edit-pem">
          ...
          >
            ...
            <span>
              {{
                t("settings.certificates.rotationWarning", {
                  siteCount: siteUsageCount,
                  siteUnit:
                    siteUsageCount === 1
                      ? t("settings.certificates.site")
                      : t("settings.certificates.sites"),
                  domainCount: domainUsageCount,
                  domainUnit:
                    domainUsageCount === 1
                      ? t("settings.certificates.dockerDomain")
                      : t("settings.certificates.dockerDomains"),
                })
              }}
            </span>
          </div>
        ...
        <SheetFooter>
          ...
          <Button type="submit" :disabled="isLoading">
            ...
            />
            {{ t("settings.certificates.saveChanges") }}
          </Button>
        </SheetFooter>
      </form>
    </SheetContent>
