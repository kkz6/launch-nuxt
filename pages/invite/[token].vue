...
onMounted(async () => {
  try {
    const response = await $fetch<{ email: string }>(`/auth/invitations/${token.value}`, {
  ...
  try {
    ...
    if (error && typeof error === 'object' && 'data' in error) {
      ...
      if (fetchError.data?.errors) {
        ...
        errors.value = { name: fetchError.data?.message || 'An error occurred' }
      }
    } else {
...
<template>
  <div class="flex min-h-screen items-center justify-center">
    ...
    <div v-else class="w-full max-w-md px-6">
      ...
      <h3 class="mb-2 text-lg font-semibold text-foreground">Complete Your Invitation</h3>
