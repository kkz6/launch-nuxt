...
type FormFields = 'name' | 'service_provider' | 'server_provider_id' | 'region' | 'plan' | 'type' | 'operating_system' | 'database' | 'php' | 'ip' | 'port'
const setStringField = (field: FormFields, value: unknown) => {
  setFieldValue(field, value != null ? String(value) : '')
}

const filteredProviders = computed(() => {
  return serverProviders.value.filter((p) => p.provider === values.service_provider)
})
  ...
  try {
    const [optionsData, providersData, sshData] = await Promise.all([
      $api<{
        ...
        plans: Record<string, CloudServiceConfig>
        canCreateServer: boolean
      }>('/servers/create-options'),
      $api<{ data: ConnectedServerProvider[] }>('/server-providers'),
      $api<{ data: SSHKey[] }>('/ssh-keys'),
    ])
...
const onSubmit = handleSubmit(async (data) => {
  ...
  try {
    const response = await $api<{ data: { id: string } }>('/servers', {
      method: 'POST',
      body: {
        ...data,
...
<template>
  <div class="space-y-6">
    ...
    <div>
      ...
      <p class="text-muted-foreground">Deploy a new server to your infrastructure</p>
    </div>
    ...
    <form v-else class="space-y-6" @submit="onSubmit">
      ...
      <Card>
        ...
        <CardContent>
          <Tabs :model-value="values.service_provider" @update:model-value="setStringField('service_provider', $event)">
      ...
      <Card v-if="values.service_provider !== 'custom_server'">
        <CardHeader>
          ...
          <CardDescription>Select your connected provider account</CardDescription>
        </CardHeader>
        <CardContent>
          <Select :model-value="values.server_provider_id" @update:model-value="setStringField('server_provider_id', $event)">
            <SelectTrigger>
              <SelectValue placeholder="Select provider account" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem v-for="provider in filteredProviders" :key="provider.id" :value="String(provider.id)">
                {{ provider.profile }} ({{ serviceProviders[provider.provider] }})
              </SelectItem>
            </SelectContent>
          </Select>
          <p v-if="filteredProviders.length === 0" class="mt-2 text-sm text-muted-foreground">
            No provider accounts connected.
            <NuxtLink to="/settings/server-providers" class="text-primary hover:underline">
              Connect a provider
            </NuxtLink>
          </p>
        </CardContent>
      ...
      <Card v-if="values.service_provider === 'custom_server'">
        <CardHeader>
          ...
          <CardDescription>Enter your server's connection details</CardDescription>
        </CardHeader>
      ...
      <Card>
        ...
        <CardContent class="space-y-4">
          ...
          <div v-if="values.service_provider !== 'custom_server'" class="grid gap-4 md:grid-cols-2">
            <div class="space-y-2">
              <Label for="plan">Server Size</Label>
              <Select :model-value="values.plan" @update:model-value="setStringField('plan', $event)">
                <SelectTrigger>
                  <SelectValue placeholder="Select size" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem v-for="plan in currentPlans" :key="plan.value" :value="plan.value">
                    {{ plan.title }}
                  </SelectItem>
                </SelectContent>
              </Select>
            ...
            <div class="space-y-2">
              ...
              <Select :model-value="values.region" @update:model-value="setStringField('region', $event)">
                <SelectTrigger>
                  <SelectValue placeholder="Select region" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem v-for="region in currentRegions" :key="region.value" :value="region.value">
                    {{ region.title }}
                  </SelectItem>
                </SelectContent>
              </Select>
          ...
          <div class="space-y-2">
            ...
            <Select :model-value="values.type" @update:model-value="setStringField('type', $event)">
              <SelectTrigger>
                <SelectValue placeholder="Select type" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem v-for="(label, key) in serverTypes" :key="key" :value="key">
                  {{ label }}
                </SelectItem>
              </SelectContent>
            </Select>
        ...
        <CardContent class="space-y-4">
          <div class="grid gap-4 md:grid-cols-2">
            <div class="space-y-2">
              ...
              <Select :model-value="values.operating_system" @update:model-value="setStringField('operating_system', $event)">
                <SelectTrigger>
                  <SelectValue placeholder="Select OS" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem v-for="(label, key) in operatingSystems" :key="key" :value="key">
                    {{ label }}
                  </SelectItem>
                </SelectContent>
              </Select>
            ...
            <div v-if="values.type !== 'database'" class="space-y-2">
              ...
              <Select :model-value="values.php" @update:model-value="setStringField('php', $event)">
                <SelectTrigger>
                  <SelectValue placeholder="Select PHP version" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem v-for="(label, key) in phpVersions" :key="key" :value="key">
                    {{ label }}
                  </SelectItem>
                </SelectContent>
              </Select>
            ...
            <div class="space-y-2">
              ...
              <Select :model-value="values.database" @update:model-value="setStringField('database', $event)">
                <SelectTrigger>
                  <SelectValue placeholder="Select database" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem v-for="(label, key) in databaseTypes" :key="key" :value="key">
                    {{ label }}
                  </SelectItem>
